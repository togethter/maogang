...
- (void)prepareLayout {
    ...
    if (self.vertical) {
        ...
        [self prepareLayoutAtScrollDirectionVerticalwww];
    }
}

- (void)prepareLayoutAtScrollDirectionVerticalwww {
    // 每次重新布局都需要清空数据
    ...
    for (NSInteger section = 0; section < sectionsCount; section ++) {
        ...
        for (int i = 0; i < _numberLineCount; i++) {// 相当于行数
            // 从头视图开始每列Cell的X是相同的
            self.maxScrollDirPositionForColumn[@(i)] = @(itemWidth);
        }
        ...
        for (NSInteger row = 0; row < rowsCount; row++) {
            ...
            for (int i = 1; i < _numberLineCount; i++) {
            ...
            // 现在知道了行数
            CGFloat  valueY = (self.interitemSpacing + itemWidth) * (currentRow)+ self.startScrollDirPosition;
            ...
            CGFloat valueX = self.numberOfColumns -  y / itemHeight;//

            CGFloat xx = self.sectionInset.left+ valueX *(self.interitemSpacing + itemWidth);
            
            // 设置当前cell布局对象的frame
            attribute.frame = CGRectMake(xx , valueY, itemWidth, itemHeight);
            // 重新设置当前列的Y值（也就是当前列cell到下个cell的值）
            //            y += self.lineSpacing + itemHeight;
            CGFloat dd = [self.maxScrollDirPositionForColumn[@(currentRow)] floatValue];
            
            self.maxScrollDirPositionForColumn[@(currentRow)] = @(itemWidth + dd);
            // 保存cell的布局对象
...
//        self.startScrollDirPosition +=  ((self.interitemSpacing  + itemWidth) * self.numberLineCount)  +self.sectionInset.bottom;
        self.startScrollDirPosition +=  ((self.interitemSpacing  + itemWidth) * self.numberLineCount);
    ...
    for (NSInteger section = 0; section < sectionsCount; section ++) {
        ...
        for (NSInteger row = 0; row < rowsCount; row++) {
            ...
            if (row == rowsCount -1) {
                ...
                // 由于下cell到下个cell的Y值，所以需要减去cell间距
//                self.startScrollDirPosition = maxY - self.lineSpacing + self.sectionInset.bottom;
                self.startScrollDirPosition = maxY - self.lineSpacing;
