...
- (void)setup
{
    ...
    _maxItem = 9;
    CGFloat width = (kScreenWidth-60 - ((_itemListCols + 1) * _itemMargin)) / _itemListCols;
...
- (void)pan:(UIPanGestureRecognizer *)pan
{
    ...
    if (pan.state == UIGestureRecognizerStateChanged) {
        ...
        if (otherButton && !otherButton.isAdd) { // 插入到当前按钮的位置
            ...
            if (_clickItemShiftPositionBlock) {
                 _clickItemShiftPositionBlock(tagButton,otherButton);
            }
...
- (UIView *)deleteView{
    if (!_deleteView) {
        ...
        button.tag = 201809;
         button.frame = CGRectMake(0, 0, kScreenWidth, _deleteViewHeight);
