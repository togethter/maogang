...
- (void)viewDidLoad {
    ...
    [self.nodataView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerX.mas_equalTo(self.mytableView);
        make.centerY.mas_equalTo(self.mytableView.mas_centerY).offset(-70);
    }];
...
- (void)networkReadMsgWithMsgID:(MessageModel *)messageModel cell:(UITableViewCell *)cell indexPath:(NSIndexPath *)indexPath{
//    MyReadMsg
    NSString *token = TOKEN;
    NSString *_msgId = messageModel.MessageId;
    if (!IS_VALID_STRING(_msgId)) {
        [AlertPool alertMessage:@"msgId为空" xlViewController:self WithBlcok:nil];
        return;
    }
    cell.userInteractionEnabled = NO;
    [self netWorkHelperWithPOST:MyReadMsg parameters:@{@"token":token,@"msgId":_msgId} success:^(id responseObject) {
        ...
        if ([model.Result isEqualToString:@"200"]) {
            ...
            if (self.messageArray.count > indexPath.row) {
              ...
              [self pushViewCollection:messageModel];
        }
    } failure:^(NSError *error) {
        cell.userInteractionEnabled = YES;
    }];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (self.messageArray.count > indexPath.row) {
        ...
        [self networkReadMsgWithMsgID:model cell:cell indexPath:indexPath];
    }
}
...
#pragma mark --- 跳转界面
-(void)pushViewCollection:(MessageModel*)model{
    if (IS_VALID_STRING(model.MsgType))  {
        switch (model.MsgType.integerValue) {
            case 1://点赞
            case 10://回复
                {
                    if (IS_VALID_STRING(model.RelativeId)) {
                        YLWorkDetailsViewController * vc = [[YLWorkDetailsViewController alloc]init];
                        vc.postId = model.RelativeId;
