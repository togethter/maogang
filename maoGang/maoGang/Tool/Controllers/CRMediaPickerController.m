...
- (void)chooseMedia
{
	...
	NSString *photoLibraryButton = NSLocalizedString(@"相册", nil);
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex
{
    ...
    } else if ([buttonTitle isEqualToString:NSLocalizedString(@"取消", nil)]) {
        if (self.delegate && [self.delegate respondsToSelector:@selector(CRMediaPickerControllerDidCancel:)]) {
            [self.delegate CRMediaPickerControllerDidCancel:self];
        }
        return;
                ...
                if (success)
                {
                    ...
                    if (imageAsset)
                    {
                        ...
                        dispatch_async(dispatch_get_main_queue(), ^{
                            if (self.delegate && [self.delegate respondsToSelector:@selector(CRMediaPickerController:didFinishPickingAsset:)]) {
                                [picker.presentingViewController dismissViewControllerAnimated:YES completion:^{
                                    [self.delegate CRMediaPickerController:self didFinishPickingAsset:imageAsset];
                                }];
                            } else {
                                [picker.presentingViewController dismissViewControllerAnimated:YES completion:NULL];
                            }
                        ...
                        dispatch_async(dispatch_get_main_queue(), ^{
                            if (self.delegate && [self.delegate respondsToSelector:@selector(CRMediaPickerController:didFinishPickingAsset:)]) {
                                [picker.presentingViewController dismissViewControllerAnimated:YES completion:^{
                                    [self.delegate CRMediaPickerController:self didFinishPickingAsset:nil];
                                }];
                            } else {
                                [picker.presentingViewController dismissViewControllerAnimated:YES completion:NULL];
                            }
...
//                if (!error) {
//
//                    [[[self class] defaultAssetsLibrary] assetForURL:assetURL resultBlock:^(ALAsset *asset) {
//
//
//                        ALAssetRepresentation *representtation = asset.defaultRepresentation;
////                        CGImageRef  imageRef = representtation.fullScreenImage;
