...
@interface YLWorkDetailsViewController ()<CircleUserCellDelegate,XLScrollowContainCommentViewDelegate>{
    NSInteger _page;
...
@implementation YLWorkDetailsViewController
...
-(void)creatUI{
    ...
    self.tableV.estimatedRowHeight = 100;//预设 cell高度
...
-(void)downLoad{
    ...
    if (IS_VALID_STRING(self.postId)) {
        ...
        [self netWorkHelperWithPOST:PostesPostDetails parameters:@{@"postId":self.postId,@"token":toekn} success:^(id responseObject) {
...
-(UIView * )tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    ...
    [headerView setContentData:item section:section withIsShowDeleteButton:NO];
    return headerView;
...
- (void)didClickDeteleButton:(NSInteger)section withButton:(UIButton *)btn{
    ...
    CircleItem * model = self.dataMuArr[section];
    [self MyDeleteCollectionToModel:model withButton:btn withSection:section];
    
}
-(void)MyDeleteCollectionToModel:(CircleItem *)model  withButton:(UIButton *)btn withSection:(NSInteger)section{
    
    if (IS_VALID_STRING(model.PostId)) {
        btn.userInteractionEnabled = NO;
        [self netWorkHelperWithPOST:MyPostesDelete parameters:@{@"token":@"f90aef2527ae914ad6af06684b1b7a42",@"postId":model.PostId} success:^(id responseObject) {
            btn.userInteractionEnabled = YES;
            BaseModel * Bmodel = [BaseModel loadModelWithDictionary:responseObject];
            if ([Bmodel.Result isEqualToString:@"200"]) {
                LRWeakSelf(self)
                [AlertPool alertMessage:Bmodel.message xlViewController:self WithBlcok:^{
