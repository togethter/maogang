...
//        _headNV = [[UIView alloc]initWithFrame:CGRectMake(0, 0, kScreenWidth, 44+22)];
        ...
        [self.headB setImage:[UIImage imageNamed:@"touxiang_moren"] forState:UIControlStateNormal];
...
//    headerView.backgroundColor = [UIColor redColor];
    ...
    if (IS_VALID_STRING(self.infoModel.HeadPic)) {
        ...
        [self.H_headB setBackgroundImage:[UIImage imageNamed:@"touxiang_moren"] forState:UIControlStateNormal];
...
-(CGFloat)tableView:(UITableView *)tableView heightForHeaderInSection:(NSInteger)section{
   ...
   CGSize officArea  = [NSString setSizeSelfH:self.infoModel.Nick withSize:CGSizeMake(kScreenWidth-28-15, MAXFLOAT) withDic:@{NSFontAttributeName:[UIFont systemFontOfSize:18]}];
    [self.Icon_backImage mas_remakeConstraints:^(MASConstraintMaker *make) {
        make.top.mas_equalTo(self.tableV).mas_offset((kScreenHeight-(titleSize.height+150+35+29+29+32+officArea.height)-self.Icon_backImage.image.size.height)/2 +(titleSize.height+150+35+29+29+32+officArea.height)-30 );
        make.centerX.mas_equalTo(self.tableV);
