...
//line 宽度 减去值
#define lINE_WIDTH_reduction 50
...
#pragma mark--修改line的宽度
    CGFloat line_w = CURRENT_WIDTH(currentIndex) ;
     CGFloat ju_top = 0;
    if (self.isDefult) {
        line_w = line_w - lINE_WIDTH_reduction;
        ju_top = 5;
    } else {
        ju_top = 0;
    }
    UIView *line = [[UIView alloc] initWithFrame:CGRectMake(0, SCROLL_HEIGHT-_lineHeight - ju_top, line_w*_lineScale, _lineHeight)];
...
- (void)changeBtnFrom:(NSInteger)from to:(NSInteger)to {
    ...
    if (lineView) {
        ...
        if (self.isDefult) {
            lineView.width = lineView.width - lINE_WIDTH_reduction;
...
- (void)changePointScale:(CGFloat)scale {
    ...
    switch (_headStyle) {
        ...
        case SegmentHeadStyleArrow:
        {
            if (lineView) {
                //lineView位置变化
                #pragma mark===动画  CURRENT_WIDTH(changeIndex)-20
