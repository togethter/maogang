...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (self.photos.count) {
        ...
        if ([self.delegate respondsToSelector:@selector(photoBrowserStateBegan:photoDidSelectView:atIndex:cellIndex:)]) {
            scrollView.callbackStateBegan  = ^(id obj){
               
                    [weakSelf.delegate photoBrowserStateBegan:weakSelf photoDidSelectView:weakScrollBoxView atIndex:indexPath.row cellIndex:self.section];
...
//        DLog(@"ZLPhotoLib提示: 您没有传photos数组");
        return;
    }
...
- (void)delete{
    ...
    UIAlertController * Atc = [UIAlertController alertControllerWithTitle:@"确定要删除此图片？" message:nil preferredStyle:UIAlertControllerStyleAlert];
    UIAlertAction * canalB = [UIAlertAction actionWithTitle:@"取消" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        
    }];
    
    [Atc addAction:canalB];
    
    UIAlertAction * sureB = [UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
    ...
    [Atc addAction:sureB];
    [self presentViewController:Atc animated:YES completion:^{
