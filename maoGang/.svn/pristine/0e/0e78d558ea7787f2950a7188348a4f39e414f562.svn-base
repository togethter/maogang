...
-(void)setContentData:(CircleItem *)circleItem section:(NSInteger)section withIsShowDeleteButton:(BOOL)IsShow{
    ...
    [self.headImgV sd_setImageWithURL:[NSURL URLWithString:circleItem.MemberHeadPic] placeholderImage:[UIImage imageNamed:@"touxiang_moren"]];
    ...
    [self setLikeBtnTitleAndIsShowDeleteButton:IsShow];//赞 收藏 文字显示  删除
...
//    tupian_jiazai
    [imgV sd_setImageWithURL:[NSURL URLWithString:urlStr] placeholderImage:[UIImage imageNamed:@"tupian_jiazai"]];
...
//            make.width.equalTo(@80);
    ...
    [self.delete_btn mas_remakeConstraints:^(MASConstraintMaker *make) {
        ...
        make.left.equalTo(self.timeLabel.mas_right).mas_offset(15);
...
#pragma mark --- 赞  收藏
-(void)setLikeBtnTitleAndIsShowDeleteButton:(BOOL)isShow {
    
    if (isShow) {//隐藏
        self.delete_btn.hidden = YES;
    }else{
        if (IS_VALID_STRING(self.item.MemberId)) {
            NSString * My_MemberId =[UserManager sharedManager].userModel.MemberId;
