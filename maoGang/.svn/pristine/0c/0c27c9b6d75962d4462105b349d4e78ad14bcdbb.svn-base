...
+ (NSURLSessionTask *)POST:(NSString *)URL
    ...
    //    NSLog(@"%@",[NSString stringWithFormat:@"%@?%@",URL,parameters]);
...
//            NSData *imageData =  images[i];
            ...
            if (IS_VALID_STRING(name)) {
                [formData appendPartWithFileData:imageData
                                            name:name
                                        fileName:fileNames ? NSStringFormat(@"%@.%@",fileNames.firstObject,imageType?:@"jpg") : imageFileName
                                        mimeType:NSStringFormat(@"image/%@",imageType ?: @"jpg")];
            } else if (IS_VALID_ARRAY(name)) {
                if ([name count] >i) {
                    [formData appendPartWithFileData:imageData
                                                name:name[i]
                                            fileName:fileNames ? NSStringFormat(@"%@.%@",fileNames.firstObject,imageType?:@"jpg") : imageFileName
...
+ (NSURLSessionTask *)downloadWithURL:(NSString *)URL
    ...
    __block NSURLSessionDownloadTask *downloadTask = [_sessionManager downloadTaskWithRequest:request progress:^(NSProgress * _Nonnull downloadProgress) {
        ...
        //拼接文件路径
        NSString *filePath = [downloadDir stringByAppendingPathComponent:[response.suggestedFilename stringByReplacingPercentEscapesUsingEncoding:NSUTF8StringEncoding]];
        
//        NSLog(@"response.suggestedFilename---%@",[response.suggestedFilename stringByReplacingPercentEscapesUsingEncoding:NSUTF8StringEncoding]);
        //返回文件位置的URL路径
